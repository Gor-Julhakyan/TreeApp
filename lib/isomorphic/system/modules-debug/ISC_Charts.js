...
if (window.isc && isc.version != "v11.1p_2017-10-10/EVAL Development Only" && !isc.DevUtil) {
...
,isc.A._getViewPortWidth=function isc_FacetChart__getViewPortWidth(ignoreOverflow){
    ...
    if(expandedWidth&&!ignoreOverflow)return expandedWidth;
    return this.invokeSuper(isc.FacetChart,"_getViewPortWidth");
}
...
,isc.A._getCheckedViewPortWidth=function isc_FacetChart__getCheckedViewPortWidth(viewPortWidth){
    ...
    if(drawingType=="bitmap"){
        var limit=isc.Browser.isIE?4000:32000;
...
,isc.A.validateAutoScrollData=function isc_FacetChart_validateAutoScrollData(){
    ...
    if(this.autoScrollData){
        ...
        }else if(chartType!="Area"&&chartType!="Line"&&chartType!="Column"){
            this.logWarn("Automatic horizontal expansion of charts requires a column, line, "+
                         "or area chart, but found a "+chartType+"'.  Expansion will "+
                         "therefore be disabled for this chart.");
        }else if(approach=="clusters"&&!this._getExpandableBarChartContext()){
            this.logWarn("Automatic horizontal expansion of charts based on cluster sizes "+
                "only works for column or histogram charts, which this chart and its "+
                "extra axes aren't.  Expansion will therefore be disabled for this chart.");
        }else{
            if(!this._autoScrollData){
...
,isc.A.initWidget=function isc_FacetChart_initWidget(){
    ...
    if(this.autoScrollData)this.validateAutoScrollData();
...
isc.evalBoundary;isc.B.push(isc.A.drawChart=function isc_FacetChart_drawChart(){
    this.beginBatchDrawing();
    try{
        var overflow=this._drawChart(this._autoScrollData);
        if(overflow){
            this._setOverflowWidth(overflow);
...
,isc.A._drawChart=function isc_FacetChart__drawChart(considerOverflow){
    ...
    if(this._usingNewLayoutAlgorithm){
        ...
        if(considerOverflow){
            var data=this.getOrderedData(),
                overflow=this._chartOverflow||0,
                chartRectWidth=this.getChartWidth(),
                allSettings=hasExtraAxes?this._getExtraAxisSettings().all:null;
            if(this.autoScrollDataApproach!="labels"){
                var context=this._getExpandableBarChartContext();
                if(context){
                    var minDataLength=this._calculateMinBarChartDataLength(context,data,0,
                                                                             data.length-1);
                    if(overflow<minDataLength-chartRectWidth){
                        overflow=minDataLength-chartRectWidth;
                    }
                }
            }
            if(overflow>0){
...
isc.evalBoundary;isc.B.push(isc.A._getPossibleIncrements=function isc_FacetChart__getPossibleIncrements(primary,minValue,maxValue,context,scaleFactor){
...
,isc.A._drawThisGradationLabel=function isc_FacetChart__drawThisGradationLabel(left,top,labelHeight,stringValue,props){
    this.drawLabel(Math.max(left,this.gradationLabelPadding),top-(labelHeight/2),stringValue,props);
}
...
isc.evalBoundary;isc.B.push(isc.A._getDataLabels=function isc_FacetChart__getDataLabels(){
...
,isc.A._calculateBarThickness=function isc_FacetChart__calculateBarThickness(availableSpace,availableBarSpace,data,start,end,barsPerCluster,context){
    ...
    if(cache)return cache.actualBarSpace;
...
,isc.A._getExpandableBarChartContext=function isc_FacetChart__getExpandableBarChartContext(){
    ...
    return this.chartType=="Column"||this.chartType=="Histogram"?this:null;
}
...
,isc.A._drawDataValueLabelForBar=function isc_FacetChart__drawDataValueLabelForBar(left,top,label,chartType,checkDataValueLabel,prevLeft,prevTop,center){
    if(chartType!="Bar"){
        left-=label.width/2;
...
,isc.A.drawBar=function isc_FacetChart_drawBar(vertical,start,base,thickness,length,props,context,zIndex){
    if(!vertical){
...
isc.evalBoundary;isc.B.push(isc.A.drawPieSeries=function isc_FacetChart_drawPieSeries(values,total,centerPoint,radius,dataIndex,innerRing){
...
isc.evalBoundary;isc.B.push(isc.A.prepareForDragging=function isc_FacetChart_prepareForDragging(){
...
,isc.A.titleBorderAndBackground=function isc_FacetChart_titleBorderAndBackground(){
        if(this.drawTitleBackground){
            var rectWidth=0;
            if(this.autoScrollData)rectWidth=this.getScrollWidth();
        ...
        if(this.drawTitleBoundary){
            ...
            if(this.autoScrollData)boundaryLineEnd=this.getScrollWidth();
...
,isc.A._legendHeightCalc=function isc_FacetChart__legendHeightCalc(numRows,swatchSize,legendPadding,legendRowPadding){
    if(this.drawLegendBoundary){
        var height;
        if(numRows>1)height=numRows*10+legendRowPadding;
        else height=0;
        height=height+this.legendRectHeight;
        return height;
    }else{
...
,isc.A._loadLegendRect=function isc_FacetChart__loadLegendRect(left,top,width,availWidth,height){
    ...
    if(this.drawLegendBoundary){
        var boundayLineEnd=this.getWidth();
        var boundaryLineStart=0;
        var boundaryLineHeight=this.getHeight()-legendRect.height-1;
...
,isc.A._getDataLineColor=function isc_FacetChart__getDataLineColor(index,facetValueId,purpose,context){
    ...
    if(this.matchBarChartDataLineColor)var color=context.getDataColor(index,facetValueId,purpose);
    else var color=context.getDataLineColor(index,facetValueId,purpose);
    return isc.DrawPane._mutergb(this.colorMutePercent,color);
}
...
,isc.A._setColorForMouseOver=function isc_FacetChart__setColorForMouseOver(drawItem,color){
                ...
                if(nearestValue.drawItemId){
                    ...
                    if(this._highlightedItem==null){
                        ...
                        if(drawItem.radius){
                            ...
                            this._setColorForMouseOver(drawItem,color);
...
isc.evalBoundary;isc.B.push(isc.A._cleanHoverItems=function isc_FacetChart__cleanHoverItems(destroy){
    ...
    return{
        ...
        utility:function labelCollapseModeNoneAxisUtility(width,height,measure){
            ...
            if(rotated&&!forceRotateLabels&&totalUnrotatedLabelLength<axisLength){
                ...
                if(chart._autoScrollData&&chart.autoScrollDataApproach!="clusters"){
                    if((chart._chartOverflow||0)<overflow)chart._chartOverflow=overflow;
                }
