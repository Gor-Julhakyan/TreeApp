...
isc.B.push(isc.A.useAJAX=function isc_c_Messaging_useAJAX(){
        return!this.useEventSource&&!this.useWebSocket&&
...
,isc.A._send=function isc_c_Messaging__send(channels,data,callback,requestProperties){
    if(this.useWebSocket&&this._conn&&window[this._conn]){
...
,isc.A._connectRetry=function isc_c_Messaging__connectRetry(){
    if(this._pendingConn&&window[this._pendingConn]){
        if(window[this._pendingConn].destroy){
            window[this._pendingConn].destroy();
        }else if(window[this._pendingConn].close){
            try{
                window[this._pendingConn].close();
            }catch(e){}
        }
    }
...
,isc.A._connect=function isc_c_Messaging__connect(){
    ...
    var url=this.useWebSocket?isc.Page.getURL(this.websocketURL).replace(/^http(s)?/i,"ws$1")
    ...
    if(this.useWebSocket){
        ...
        commFrame.destroy();
        commFrame=null;
        if(this._lastWebSocketId==null){
            this._lastWebSocketId=0;
        }else{
            this._lastWebSocketId++;
        }
        data.connectionID="websocket"+this._lastWebSocketId;
        this._pendingConn=data.connectionID;
        for(var fieldName in data){
            if(!data.hasOwnProperty(fieldName))continue;
            uriBuilder.setQueryParam(fieldName,String(data[fieldName]));
        }
        var webSocket=window[this._pendingConn]=new WebSocket(uriBuilder.uri);
        this._webSocketInitTimer=isc.Timer.setTimeout(function(){
            ...
            _this._pendingConn=window[_this._pendingConn]=null;
        ...
        webSocket.onerror=function(event){
            ...
            if(_this._webSocketKnownWorking){
                _this.logDebug("websocket known working - continuing to retry");
            }else{
...
,isc.A._connectCallback=function isc_c_Messaging__connectCallback(conn,config){
    ...
    if(this._conn&&window[this._conn]){
        if(window[this._conn].destroy){
            window[this._conn].destroy();
        }else if(window[this._conn].close){
            try{
                window[this._conn].close();
            }catch(e){}
        }
    }
...
,isc.A._destroyConns=function isc_c_Messaging__destroyConns(){
    if(this._pendingConn!=null&&window[this._pendingConn]){
        if(window[this._pendingConn].destroy){
            window[this._pendingConn].destroy();
        }else if(window[this._pendingConn].close){
            try{
